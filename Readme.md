...
- The raw output in binary format in `xxd.log`. Result of touching the 4 corners of the screen.
...
```xxd -b /dev/hidraw0```
 ...
 At first, I did not have the xorg evdev driver `xf86-input-evdev` installed. So after putting the `52-egalax-usbhid.conf` in `/usr/share/X11/xorg.conf.d` you can see in the log that it falls back on libinput, which in turn does not seem to like the touchscreen since it's missing some information. The screen might just be too old for libinput.
