...
fn list_devices() -> libusb::Result<()> {
    ...
    for device in context.devices()?.iter() {
        ...
        // a.d. egalax
        if !(device_desc.vendor_id() == 0x0eef && device_desc.product_id() == 0x0001) {
            continue;
        }
            ...
            for interface in config_desc.interfaces() {
                for interface_desc in interface.descriptors() {
                    ...
                    for endpoint_desc in interface_desc.endpoint_descriptors() {
                        ...
                        // );
                        // let handle = &usb_device.as_ref().unwrap().handle;
                        // let buf = [0x0a, 0x01, 0x41];
                        // let res =
                        //     handle.write_control(rq, 0x00, 0, 0, &buf, Duration::from_secs(1));
                        // println!("Result: {:?}", res);
                        // println!("rq {:08b}", rq);

                        let mut buf2 = [0; 2];
                        let res2 = handle.read_control(
                            130,
                            0x00,
                            0,
                            0 | (1 << 7) | 1,
                        ...
                        println!("Result {:?}, answer {:?}", res2, buf2);
                    }
                }
            }
        }
